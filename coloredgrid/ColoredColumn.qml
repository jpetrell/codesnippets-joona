...
Column {
    ...
    Repeater {
        ...
        Rectangle {
            ...
            property real blue: value < 0.4 ? 0.8+0.2*Math.random() : 0.1+0.4*Math.random()
            property real red: value > 0.4 && value < 0.8 ? 0.7+0.3*Math.random() : 0.4+0.3*Math.random()
            property real green: value > 0.8 ? 0.2+0.2*Math.random() : 0.2*Math.random()
        }
    }
}
